...
public class FinancialBillDetailActivity extends BaseActivity {
...
//        Log.i("hh", "mType = " + mType);
    ...
    private void requestData() {
        ...
        RemoteFactory.getInstance().getProxy(CommonRequestProxy.class)
                .getFinancialDetailData(paramWrapper)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new CommonObserverAdapter<String>() {
            @Override
            public void onMyError() {
                ToastUtil.showCustom("理财账单详情数据获取失败");
            }
    ...
    private void setData(FundBillDetailDataModel data) {
        ...
        if ("true".equals(flag)) {
            ...
            if ("CPDJ".equals(type)) { // 买入
                tv_title_category.setText("勘设联名卡-" + productName);
            } else if ("CPJD".equals(type)) { // 流标
                tv_title_category.setText("勘设联名卡-" + productName);
            } else {
                if ("CPHK".equals(type)) { // 回款
                    tv_title_category.setText(productName + "-勘设联名卡");
                }
            }
        }
        ...
        if (mType.equals("CPDJ")) { // 理财买入
            tv_turn_into_or_redeem.setText("买入到");
        } else if (mType.equals("CPHK")) { // 回款到
            tv_turn_into_or_redeem.setText("回款到 0");
        }
